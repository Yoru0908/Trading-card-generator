...
const translations = {
    en: {
        ...
        disclaimerText: "Content generated by this tool is for personal learning and entertainment only. Commercial use or public distribution is prohibited.<br>All fonts used are free materials with no commercial fonts. Using this tool indicates your agreement to the relevant terms.",
    ...
    ja: {
        ...
        disclaimerText: "このツールで生成されたコンテンツは、個人的な学習と娯楽のみを目的としています。商業利用や公開配布は禁止されています。<br>使用されているフォントはすべて無料素材で、商用フォントは含まれていません。このツールの使用は関連する利用規約への同意を示します。",
    ...
    zh: {
        ...
        disclaimerText: "本工具生成的内容仅供个人学习和娱乐使用，不得用于商业用途或公开传播。<br>所使用字体均为免费素材，无商用字体。使用本工具即表示您同意相关条款。",
        privacyPolicy: "隐私政策",
        termsOfUse: "使用条款"
    }
};
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const update = {
        ...
        fontFamily() {
            ...
            // 检查是否选择了Kosugi Maru字体，如果是则应用调整
            this.applyKosugiMaruAdjustments();
        },

        applyKosugiMaruAdjustments() {
            const isThemeKosugi = ui.themeFontFamily.value.includes('Kosugi Maru');
            const isNameKosugi = ui.nameFontFamily.value.includes('Kosugi Maru');

            // 为主题字体应用调整
            if (isThemeKosugi) {
                ui.previewTheme.classList.add('kosugi-maru-font');
                ui.previewThemeLine2.classList.add('kosugi-maru-font');
            } else {
                ui.previewTheme.classList.remove('kosugi-maru-font');
                ui.previewThemeLine2.classList.remove('kosugi-maru-font');
            }

            // 为姓名和罗马字字体应用调整
            if (isNameKosugi) {
                ui.previewName.classList.add('kosugi-maru-font');
                ui.previewRomaji.classList.add('kosugi-maru-font');
            } else {
                ui.previewName.classList.remove('kosugi-maru-font');
                ui.previewRomaji.classList.remove('kosugi-maru-font');
            }
        },
        updateLetterSpacing() {
    ...
    const init = () => {
        ...
        update.groupTheme();
        update.fontFamily();
        update.applyKosugiMaruAdjustments();
